...
const MastersScheduleForm: FC<IMastersScheduleFormProps> = props => {
	...
	const { data: scheduleData, refetch } = useQuery({
		queryKey: ['SCHEDULE'],
		queryFn: () => masterScheduleApi.getList(activeMaster?.data.id!),
		enabled: !!activeMaster?.data.id,
	});
			...
			{buttonActionType !== buttonActionTypeEnum.DELETE && (
				<Box padding={3}>
					...
					<Button
						w={'full'}
						mt={3}
						onClick={() => setFormValue('freeTime', [...watch('freeTime'), '12:00-13:00'])}
					>
